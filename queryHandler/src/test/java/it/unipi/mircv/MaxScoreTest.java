...
public class MaxScoreTest {
    ...
    public void testMaxScore() throws IOException {
        ...
        //test the max score algorithm for TFIDF
        for(int i = 0; i < queries.length; i++){ //for each query
            String query = Preprocesser.processCLIQuery(queries[i]); //process the query
            //perform the max score algorithm
            LinkedList<Map.Entry<Integer, Double>> results = MaxScore.maxScoreQuery(query, 5, false);
        ...
        //test the max score algorithm for BM25
        for(int i = 0; i < queries.length; i++){ //for each query
            String query = Preprocesser.processCLIQuery(queries[i]); //process the query
            //perform the max score algorithm
            LinkedList<Map.Entry<Integer, Double>> results = MaxScore.maxScoreQuery(query, 5, true);
            for(int j = 0; j < results.size(); j++){ //for each returned document
                Map.Entry<Integer, Double> result = results.get(j); //get the j-th document
    ...
    //setup the expected results for TFIDF
    ...
    //setup the expected results for BM25
